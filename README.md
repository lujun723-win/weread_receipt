...
同步时会读取月度阅读统计，并按本地日期生成月历。

微信读书月度数据能明确对应到日期的书，会用封面填充对应日期。同一个阅读段会沿用最近识别出的封面；如果整个月没有日期级图书信息，就用本月读得最久的书作为阅读日封面。
